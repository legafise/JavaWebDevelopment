 ...
 * @see Command
 */
public class BlockBillByIdCommand implements Command {
    @Override
    public View execute(Request request) throws CommandException {
        try {
            ...
            boolean result = ServiceFactory.getInstance().getBillService().blockBillById(billId);
            billReporter.reportBillBlockingResult(result);
