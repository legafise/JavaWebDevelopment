...
public class CalculateClientTotalPositiveBillsAmountCommand implements Command {
    ...
    public View execute(Request request) throws CommandException {
        try {
            ...
            BigDecimal totalPositiveAmount = ServiceFactory.getInstance().getClientService()
                    .calculateTotalClientPositiveBillsAmount(clientId);
            clientReporter.reportClientTotalPositiveBillsAmountInfo(totalPositiveAmount, clientId);
